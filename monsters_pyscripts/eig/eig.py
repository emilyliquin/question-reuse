# Copied from https://github.com/anselmrothe/EIG, Copyright (c) 2018, MIT license

import numpy as np
from .monsters import Parser, Executor, ObjectSetHypothesisSpace
from .hypothesis import HypothesisSpace
from .context import Context
from .bayes import Bayes, Distribution
...
def entropy(x):
    return - sum([p * log2(p) for p in x if p!=0]) # have to remove zero probabilities
...
def compute_eig_fast(program: str, board: np.ndarray, **kwargs):
    ...
    question = Parser.parse(program)
    executor = Executor(question)
    hypotheses = ObjectSetHypothesisSpace(**kwargs, observation=board)
    if len(hypotheses) == 0:
        return 0
